package com.ezen.shop.common.utils;
...
//		return str.replace("-", File.separator); => \ 로 나옴
		return str.replace("-", "/");	//"2024/11/18"
	}
	...
	public String uploadFile(String uploadFolder, String dateFolder, MultipartFile uploadFile) {
		...
		// 파일업로드작업
		try {
			File saveFile = new File(file, realUploadFileName);	// c:dev/upload/pds/2024/11/18	2f48f241-9d64-4d16-bf56-70b9d4e0e79a
			uploadFile.transferTo(saveFile); // 파일복사 -- 파일업로드(편의상 업로드라고 함)
			
			// 썸네일 작업을 위한 이미지 파일체크.
			if(checkImageType(saveFile)) { // 이미지파일
				
				// 썸네일작업을 위한 File객체. pom.xml 파일에 썸네일 기능을 제공하는 의존성 구문 존재함
